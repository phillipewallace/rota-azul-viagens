...
router.post('/login', async (req, res) => {
  try {
    ...
    if (isBcryptHash(user.password)) {
      ...
      // Legado: senha em texto plano. Compara e, se bater, migra para bcrypt silenciosamente.
      passwordMatch = password === user.password;
      if (passwordMatch) {
        try {
          const hashed = await bcrypt.hash(password, BCRYPT_ROUNDS);
          await pool.query('UPDATE users SET password = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [hashed, user.id]);
          console.log(`🔐 [AUTH] Senha migrada para bcrypt: ${user.username}`);
        } catch (e) {
          console.error('⚠️  [AUTH] Falha ao migrar hash:', e);
        }
      }
    }
