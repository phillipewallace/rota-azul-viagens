...
import { DollarSign, Loader2, Download, RefreshCw, Receipt as ReceiptIcon, CalendarDays, CheckCircle2, AlertCircle, Filter, Plus, Trash2, Wrench, TrendingDown, TrendingUp, Search, AlertTriangle, Pencil, MoreVertical, XCircle, Repeat, Tag, PlayCircle, BarChart3, FileSpreadsheet, Users2, TimerOff, X, Eye, Copy, FileText, } from 'lucide-react';
...
import { receiptsService, receiptsExtraService, expensesService, expenseCategoriesService, recurringExpensesService, contractsService, } from '@/services/contracts';
import { erpService } from '@/services/erp';
import { uploadSignedPdf } from '@/services/erp';
import { invoicesService, INVOICE_FORMA_LABEL, } from '@/services/invoices';
import { VincularNfDialog } from '@/components/erp/VincularNfDialog';
import { medicoesService } from '@/services/medicoes';
...
const nextDueDate = (competencia, diaVencimento) => {
...
const dueDateInComp = (competencia, diaVencimento) => {
    const [y, m] = (competencia || '').split('-').map(Number);
    if (!y || !m)
        return '';
...
const computeCompetenciaPeriodo = (dataInicioContrato, competencia) => {
...
const ErpFinanceiro = () => {
    ...
    const [kpisRecibos, setKpisRecibos] = useState(null);
    // filtros
    const [filterStatus, setFilterStatus] = useState('all');
    const [filterCompanyId, setFilterCompanyId] = useState('all');
    const [filterFrom, setFilterFrom] = useState('');
    const [filterTo, setFilterTo] = useState('');
    const [dateBase, setDateBase] = useState('emissao');
    ...
    const [nfDialogTarget, setNfDialogTarget] = useState(null);
    ...
    const [nfStatus, setNfStatus] = useState('all');
    const [nfForma, setNfForma] = useState('all');
    const [nfCompanyId, setNfCompanyId] = useState('all');
    ...
    const [nfKpis, setNfKpis] = useState(null);
    const [nfExportBusy, setNfExportBusy] = useState(false);
    const [nfCancelTarget, setNfCancelTarget] = useState(null);
    ...
    const [medicaoEditing, setMedicaoEditing] = useState(null);
    const [medicaoViewId, setMedicaoViewId] = useState(null);
    ...
    const [medKpis, setMedKpis] = useState(null);
    const [medExportBusy, setMedExportBusy] = useState(false);
    const [medicoesPrevMonthTotal, setMedicoesPrevMonthTotal] = useState(null);
    ...
    const pendentesScrollRef = useRef(null);
    const recibosScrollRef = useRef(null);
    // diálogos
    const [payDialog, setPayDialog] = useState(null);
    const [cancelDialog, setCancelDialog] = useState(null);
    const [reabrirDialog, setReabrirDialog] = useState(null);
    const [editVencDialog, setEditVencDialog] = useState(null);
    ...
    const [pendCompanyId, setPendCompanyId] = useState('all');
    const [pendVencFrom, setPendVencFrom] = useState('');
    const [pendVencTo, setPendVencTo] = useState('');
    const [pendQuick, setPendQuick] = useState('none');
    ...
    const loadPendentes = useCallback(async () => {
        ...
        try {
            ...
            if (!mountedRef.current || requestId !== pendentesRequestRef.current)
                return;
    ...
    const recibosBaseFilters = useMemo(() => {
        ...
        const semValidadeParam = activeTab === 'sem-validade' ? true
     ...
     */
    const fetchAllFilteredRecibos = useCallback(async (hardLimit = 5000) => {
        const PAGE = 200;
    ...
    useEffect(() => {
        ...
        if (activeTab === 'pendentes' && previous !== 'pendentes')
            void loadPendentes();
    ...
    const fetchAllFilteredInvoices = useCallback(async (hardLimit = 5000) => {
        const PAGE = 200;
    ...
    useEffect(() => {
        const [y, m] = (competencia || '').split('-').map(Number);
        if (!y || !m) {
            setMedicoesPrevMonthTotal(null);
            return;
        }
    ...
    useEffect(() => {
        ...
        expensesService.list({ from, to, origem: 'all' })
            ...
            setGastosMes(list.reduce((a, e) => a + Number(e.valor || 0), 0)); })
            .catch(() => { if (!cancelled)
            setGastosMes(0); });
        return () => { cancelled = true; };
    }, [competencia, recibos]);
    ...
    const recibosFiltrados = useMemo(() => {
        ...
        return recibos.filter(r => {
            if (r.semValidade)
                return false; // aba própria "Sem validade"
            ...
            if (filterCompanyId !== 'all') {
                ...
                if (!(r.companyRazaoSocial || '').toLowerCase().includes(target))
                    return false;
            }
            const dataRef = (dateBase === 'vencimento' ? r.dataVencimento : r.dataEmissao) || '';
            if (filterFrom && dataRef < filterFrom)
                return false;
            if (filterTo && dataRef > filterTo)
                return false;
            if (quick === 'vencidos') {
                if (r.status !== 'aberto' && r.status !== 'parcial')
                    return false;
                if (!r.dataVencimento || r.dataVencimento >= today)
                    return false;
            }
            if (quick === 'em7') {
                if (r.status !== 'aberto' && r.status !== 'parcial')
                ...
                if (!r.dataVencimento)
                    return false;
                if (r.dataVencimento < today || r.dataVencimento > venceAte)
                    return false;
            }
            if (term) {
    ...
    const pagosDoMes = useMemo(() => {
        const dentro = recibos.filter(r => !r.semValidade &&
    ...
    const pagosKpis = useMemo(() => {
        const totalRecebido = pagosDoMes.reduce((s, r) => s + Number(r.valorPago ?? (r.status === 'pago' ? r.valor : 0) ?? 0), 0);
    ...
    // ===== ações =====
    const generateOne = async (contractId, valor, opts) => {
        ...
        if (!opts?.silent)
            toast.success(`Recibo ${out.numero} gerado`);
        return out;
    };
    const gerar = async (p, opts) => {
    ...
    const gerarPeriodo = async (p, periodoInicio, periodoFim, opts) => {
        ...
        try {
            ...
            if (msg.toLowerCase().includes('já existe'))
                toast.warning('Já existe um recibo para o mês desse período.');
            else
                toast.error(msg || 'Falha ao gerar recibo');
        }
        finally {
            setWorking(null);
        }
    ...
    const gerarLote = async () => {
        ...
        for (const p of alvos) {
            try {
                await generateOne(p.contractId, Number(p.valorMensal), { semPdf: true, silent: true });
                ok++;
            }
            catch {
                fail++;
        ...
        await load();
        if (fail === 0)
            toast.success(`${ok} recibo(s) gerados como pagos`);
        else
    ...
    const pendentesFiltrados = useMemo(() => {
        ...
        return pendentes.filter((p) => {
            if (pendCompanyId !== 'all' && String(p.companyId || '') !== pendCompanyId)
                return false;
            if (q) {
                const hay = [
            ...
            if (pendVencFrom || pendVencTo || pendQuick !== 'none') {
                ...
                if (!venc)
                    return true;
                if (pendVencFrom && venc < pendVencFrom)
                    return false;
                if (pendVencTo && venc > pendVencTo)
                    return false;
                if (pendQuick === 'vencidos' && !(venc < t))
                    return false;
                if (pendQuick === 'em7') {
    ...
    const pendFiltroAtivo = !!pendSearch || pendCompanyId !== 'all' || !!pendVencFrom || !!pendVencTo || pendQuick !== 'none';
    const clearPendFilters = () => {
        setPendSearch('');
        setPendCompanyId('all');
        setPendVencFrom('');
        setPendVencTo('');
        setPendQuick('none');
    };
    ...
    const unifiedGroup = useMemo(() => {
        ...
        if (!cId || !kId)
            return null;
        if (!arr.every(p => p.companyId === cId && p.customerId === kId))
            return null;
    ...
    const gerarUnificado = async () => {
        ...
        try {
            ...
            if (failCount === 0)
                toast.success(`Recibo unificado gerado · ${okCount} contratos · ${formatComp(competencia)}`);
            else
                toast.warning(`PDF gerado. ${okCount} recibos ok, ${failCount} falharam`);
        }
        catch (e) {
    ...
    const baixar = async (r) => {
        try {
            ...
            const group = (r.periodoInicio && r.periodoFim)
                ? recibos.filter(x => x.periodoInicio === r.periodoInicio &&
    ...
    const voltarParaPendentes = async (r) => {
    ...
    const toggleSelAllRec = () => {
        if (selectedRecibos.size === recibosFiltrados.length)
            setSelectedRecibos(new Set());
        else
            setSelectedRecibos(new Set(recibosFiltrados.map(r => r.id)));
    };
    const batchMarkPaid = async () => {
        const alvos = recibosFiltrados.filter(r => selectedRecibos.has(r.id) && r.status !== 'cancelado' && r.status !== 'pago');
        if (alvos.length === 0) {
            toast.info('Nenhum recibo elegível na seleção.');
            return;
        }
    ...
    const batchReopen = async () => {
        ...
        if (ids.length === 0) {
            toast.info('Nenhum recibo pago/parcial na seleção.');
            return;
        }
        setBatchWorking(true);
        let ok = 0, fail = 0;
        for (const id of ids) {
            try {
                await receiptsExtraService.togglePaid(id, false, { valorPago: 0 });
                ok++;
            }
            catch {
                fail++;
    ...
    const batchCancelSubmit = async () => {
        ...
        if (ids.length === 0) {
            toast.info('Nenhum recibo elegível.');
            return;
        }
        setBatchWorking(true);
        let ok = 0, fail = 0;
        for (const id of ids) {
            try {
                await receiptsService.cancel(id, motivo);
                ok++;
            }
            catch {
                fail++;
    ...
    const [unifPreview, setUnifPreview] = useState(null);
    const handleUnifiedAction = async (semValidade) => {
        const alvos = pendentes.filter(p => selected.has(p.contractId));
        if (alvos.length === 0)
            return;
        const first = alvos[0];
    ...
    const confirmUnified = async (input) => {
        setBatchWorking(true);
        try {
            const results = [];
    ...
    const exportRecibosZip = async () => {
        ...
        try {
            ...
            const filtrados = lista.filter(r => r.status !== 'cancelado' &&
    ...
    return (<div className="p-4 md:p-6 lg:p-8 w-full max-w-[1400px] mx-auto space-y-6">
      <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        ...
        <div className="flex items-end gap-2">
          <div>
            ...
            <Input type="month" value={competencia} onChange={(e) => setCompetencia(e.target.value)} className="h-9 w-[160px]"/>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <button type="button" onClick={() => { setQuick(quick === 'vencidos' ? 'none' : 'vencidos'); setFilterStatus('all'); }} className="text-left rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 focus-visible:ring-offset-2 transition-transform duration-200 hover:-translate-y-0.5" title="Filtrar apenas recibos vencidos">
          <KPI label="Vencidos" value={BRL(totals.vencidos)} sub={totals.vencidosCount > 0
            ? `${totals.vencidosCount} recibo(s) em atraso`
            : 'Nenhum em atraso 🎉'} icon={TimerOff} accent="from-rose-600 to-red-700"/>
        </button>
        <KPI label="Inadimplência" value={`${totals.inadimp.toFixed(1)}%`} sub={`${BRL(totals.aberto + totals.pendente)} a receber`} icon={AlertTriangle} accent="from-rose-500 to-orange-500"/>
        <KPI label="Ticket médio" value={BRL(totals.ticket)} sub={`${totalRecibos} recibos no filtro`} icon={ReceiptIcon} accent="from-sky-500 to-indigo-600"/>
        <KPI label="Resultado do mês" value={BRL(totals.resultado)} sub={`recebido − ${BRL(gastosMes)} de gastos`} icon={totals.resultado >= 0 ? TrendingUp : TrendingDown} accent={totals.resultado >= 0 ? 'from-emerald-500 to-green-600' : 'from-rose-500 to-red-600'}/>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v)} className="w-full">
        ...
        <TabsContent value="pendentes">
          <Card>
              ...
              <div className="flex flex-wrap gap-3 items-end">
                <div className="flex-1 min-w-[200px]">
                  ...
                  <div className="relative">
                    ...
                    <Input value={pendSearch} onChange={e => setPendSearch(e.target.value)} className="h-9 pl-7" placeholder="nº contrato, cliente, empresa, CNPJ/CPF…"/>
                  </div>
                </div>
                <div>
                  <Label className="text-xs">Empresa</Label>
                  <SearchableSelect value={pendCompanyId} onValueChange={setPendCompanyId} triggerClassName="h-9 w-[200px]" placeholder="Empresa" searchPlaceholder="Buscar empresa..." options={[
            ...
            ...companies.map(c => ({ value: c.id, label: c.razaoSocial })),
                ...
                {pendFiltroAtivo && (<Button variant="ghost" size="sm" onClick={clearPendFilters}>Limpar</Button>)}
                <Button variant="outline" size="sm" onClick={exportPendentesCsv} disabled={pendentesFiltrados.length === 0} title="Exportar CSV dos pendentes exibidos (respeitando os filtros)">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {pendFiltroAtivo && (<span className="ml-auto text-xs text-muted-foreground self-center">
            ...
            : 'Selecione contratos para gerar recibos em lote (marca como pagos, sem PDF).'}
              ...
              <div className="flex items-center gap-2">
                {selected.size > 0 && (<Button size="sm" variant="ghost" onClick={() => setSelected(new Set())}>Limpar</Button>)}
                {unifiedGroup && (<Popover open={unifOpen} onOpenChange={(o) => {
                if (working === '__unified__')
                    return;
                setUnifOpen(o);
            }}>
                    <PopoverTrigger asChild>
                      <Button size="sm" disabled={working === '__unified__'} className="bg-primary text-primary-foreground hover:bg-primary/90 focus-visible:ring-2 focus-visible:ring-ring transition-colors duration-200" title="Gera um único PDF somando todos os contratos selecionados">
                        {working === '__unified__'
                    ...
                    <PopoverContent align="end" className="w-[340px] p-0 overflow-hidden border-border/60 shadow-lg">
                      ...
                      <div className="p-4 space-y-3">
                        ...
                        <div className="flex items-center justify-end gap-2 pt-0.5">
                          ...
                          <Button size="sm" onClick={() => gerarUnificado()} disabled={working === '__unified__'} className="h-8 bg-primary text-primary-foreground hover:bg-primary/90 focus-visible:ring-2 focus-visible:ring-ring transition-colors duration-200">
                            {working === '__unified__'
                ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1"/>
                : <ReceiptIcon className="h-3.5 w-3.5 mr-1"/>}
                            Gerar
                ...
                <Button size="sm" disabled={selected.size === 0 || working === '__batch__'} onClick={gerarLote} className="bg-emerald-600 hover:bg-emerald-700">
                  {working === '__batch__'
            ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1"/>
            ...
            <CardContent className="p-0">
              <div ref={pendentesScrollRef} className={`overflow-auto ${pendentesFiltrados.length > 50 ? 'max-h-[70vh]' : ''}`}>
                <Table>
                  <TableHeader className="sticky top-0 z-10 bg-card">
                    <TableRow>
                      <TableHead className="w-10">
                        <Checkbox aria-label="Selecionar todos" checked={pendentesFiltrados.length > 0 &&
            pendentesFiltrados.every(p => selected.has(p.contractId))} onCheckedChange={toggleSelAll}/>
                      </TableHead>
                  ...
                  <TableBody>
                    {pendentesFiltrados.length === 0 && (<TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                        {pendentesLoading
                ...
                : pendFiltroAtivo
                    ...
                    <VirtualRows scrollRef={pendentesScrollRef} items={pendentesFiltrados} colSpan={7} estimateSize={56} getKey={(p) => p.contractId} renderRow={(p) => (<TableRow key={p.contractId} data-state={selected.has(p.contractId) ? 'selected' : undefined}>
                          <TableCell>
                            <Checkbox aria-label={`Selecionar ${p.contractNumero}`} checked={selected.has(p.contractId)} onCheckedChange={() => toggleSel(p.contractId)}/>
                          </TableCell>
                          ...
                          <TableCell className="text-right whitespace-nowrap space-x-1">
                            <Button size="sm" variant="ghost" onClick={() => setViewContractId(p.contractId)} title="Ver contrato" aria-label="Ver contrato">
                              <Eye className="h-3.5 w-3.5"/>
                            </Button>
                            <Button size="sm" variant="outline" onClick={() => setNfDialogTarget(p)} disabled={working === p.contractId} title="Vincular Nota Fiscal emitida no portal do governo" className="border-indigo-300 text-indigo-700 hover:bg-indigo-50 hover:text-indigo-800">
                              <FileText className="h-3.5 w-3.5 mr-1"/> Vincular NF
                            </Button>
                            <GerarReciboPopover pending={p} working={working === p.contractId} onConfirm={(semValidade, dataVencimento, periodoOverride) => {
                void gerar(p, {
                    semValidade,
                    dataVencimento,
                    periodo: periodoOverride || computeCompetenciaPeriodo(p.dataInicio, competencia)
                              ...
                              <Button size="sm" disabled={working === p.contractId} className="bg-emerald-600 hover:bg-emerald-700 focus-visible:ring-emerald-500 transition-colors duration-200">
        ...
        <TabsContent value="pagos">
          <Card>
            ...
            const Icon = k.icon;
            return (<div key={k.label} className="rounded-lg border border-border bg-card p-3 transition-colors duration-200 hover:bg-muted/40">
              ...
              {/* Empty state */}
              {pagosDoMes.length === 0 && !loading && (<div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-border bg-muted/20 py-10 text-center">
                  ...
                  <Button size="sm" variant="outline" onClick={() => setActiveTab('pendentes')} className="transition-colors duration-200">
                    Ir para Pendentes
                  </Button>
                </div>)}

              {/* Desktop table */}
              {pagosDoMes.length > 0 && (<div className="hidden md:block rounded-lg border border-border overflow-hidden">
                  <Table>
                    <TableHeader>
                ...
                const dueTone = dd == null ? 'bg-muted text-muted-foreground'
                    : dd < 0 ? 'bg-destructive/10 text-destructive'
                        : dd <= 7 ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400'
                            : 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400';
                return (<TableRow key={r.id} className="transition-colors duration-150 hover:bg-muted/40">
                            ...
                            <TableCell>
                              ...
                              {r.customerDocument && (<div className="text-xs text-muted-foreground">{r.customerDocument}</div>)}
                            </TableCell>
                            <TableCell className="text-right">
                              <div className="font-medium text-foreground">{BRL(pago)}</div>
                              {parcial && (<div className="text-xs text-muted-foreground">de {BRL(Number(r.valor || 0))}</div>)}
                            </TableCell>
                            <TableCell>
                              {r.formaPagamento ? (<Badge variant="secondary" className="font-normal">
                            ...
                            <TableCell>
                              <div className="flex flex-col gap-1">
                                ...
                                {dd != null && (<span className={`inline-flex w-fit items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${dueTone}`}>
                            ...
                            <TableCell className="text-right">
                              <div className="flex justify-end gap-1">
                                <Button size="sm" variant="ghost" className="h-8 transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-ring/50" onClick={() => setViewContractId(r.contractId)} aria-label="Ver contrato" title="Ver contrato">
                                  <Eye className="h-3.5 w-3.5"/>
                                </Button>
                                <Button size="sm" variant="ghost" className="h-8 transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-ring/50" onClick={() => generateReceiptPdf(receiptForPdf(r))} aria-label="Baixar PDF do recibo">
                                  <Download className="h-3.5 w-3.5"/>
                                </Button>
                                <Button size="sm" variant="ghost" className="h-8 transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-ring/50" onClick={() => setCancelDialog(r)} aria-label="Cancelar recibo">
              ...
              {/* Mobile cards */}
              {pagosDoMes.length > 0 && (<div className="md:hidden space-y-2">
                  {pagosDoMes.map((r) => {
                const parcial = r.status === 'parcial';
                const pago = Number(r.valorPago ?? (r.status === 'pago' ? r.valor : 0) ?? 0);
                const nd = nextDueDate(competencia, Number(r.diaVencimento || 10));
                ...
                const dueTone = dd == null ? 'bg-muted text-muted-foreground'
                    : dd < 0 ? 'bg-destructive/10 text-destructive'
                        : dd <= 7 ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400'
                            : 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400';
                return (<div key={r.id} className="rounded-lg border border-border bg-card p-3 transition-colors duration-200 hover:bg-muted/40">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="text-right">
                            ...
                            {parcial && (<div className="text-[11px] text-muted-foreground">parcial</div>)}
                          </div>
                        </div>
                        <div className="mt-2 flex items-center justify-between gap-2">
                          ...
                          {dd != null && (<span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${dueTone}`}>
                              {dd < 0 ? `vencido há ${Math.abs(dd)}d` : dd === 0 ? 'hoje' : `em ${dd}d`}
                            </span>)}
                        </div>
                        <div className="mt-2 flex items-center justify-between gap-2">
                          {r.formaPagamento ? (<Badge variant="secondary" className="font-normal">
                              {FORMA_LABEL[r.formaPagamento]}
                            </Badge>) : <span className="text-xs text-muted-foreground">sem forma</span>}
                          <div className="flex gap-1">
                            <Button size="sm" variant="ghost" className="h-8 transition-colors duration-200" onClick={() => generateReceiptPdf(receiptForPdf(r))} aria-label="Baixar PDF do recibo">
                              <Download className="h-3.5 w-3.5"/>
                            </Button>
                            <Button size="sm" variant="ghost" className="h-8 transition-colors duration-200" onClick={() => setCancelDialog(r)} aria-label="Cancelar recibo">
                              <XCircle className="h-3.5 w-3.5"/>
                            </Button>
                          </div>
                        </div>
        ...
        <TabsContent value="emitidos">
          <Card>
            <CardContent className="p-4 space-y-3 border-b">
              <div className="flex flex-wrap gap-3 items-end">
                <div className="flex-1 min-w-[200px]">
                  ...
                  <div className="relative">
                    <Search className="h-3.5 w-3.5 text-slate-400 absolute left-2 top-1/2 -translate-y-1/2 pointer-events-none"/>
                    <Input value={search} onChange={e => setSearch(e.target.value)} className="h-9 pl-7" placeholder="nº recibo, contrato, cliente, empresa…"/>
                  </div>
                </div>
                <div>
                  <Label className="text-xs">Base</Label>
                  <SearchableSelect value={dateBase} onValueChange={(v) => setDateBase(v)} triggerClassName="h-9 w-[140px]" placeholder="Base" options={[
            ...
            { value: 'vencimento', label: 'Vencimento' },
                ...
                <div>
                  <Label className="text-xs">Empresa</Label>
                  <SearchableSelect value={filterCompanyId} onValueChange={setFilterCompanyId} triggerClassName="h-9 w-[200px]" placeholder="Empresa" searchPlaceholder="Buscar empresa..." options={[
            { value: 'all', label: 'Todas' },
            ...companies.map(c => ({ value: c.id, label: c.razaoSocial })),
        ]}/>
                </div>
                <div>
                  ...
                  <SearchableSelect value={filterStatus} onValueChange={(v) => setFilterStatus(v)} triggerClassName="h-9 w-[140px]" placeholder="Status" options={[
            ...
            { value: 'cancelado', label: 'Cancelados' },
                ...
                <Button variant="outline" size="sm" onClick={exportAllFilteredCsv} disabled={exportBusy !== null} className="border-emerald-200 text-emerald-700 hover:bg-emerald-50 hover:text-emerald-800 dark:border-emerald-800/60 dark:text-emerald-400 dark:hover:bg-emerald-950/40 transition-colors duration-200" title="Exportar CSV de TODOS os recibos que casam com os filtros atuais (dataset completo, não só a página)">
                  {exportBusy === 'csv'
            ? <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin"/>
            : <FileSpreadsheet className="h-3.5 w-3.5 mr-1"/>}
                  Exportar CSV (filtro)
                </Button>
                <Button variant="outline" size="sm" onClick={exportAllFilteredZip} disabled={exportBusy !== null} className="border-primary/30 text-primary hover:bg-primary/10 hover:text-primary dark:border-primary/40 transition-colors duration-200" title="Baixar ZIP com PDFs de TODOS os recibos do filtro atual (não só a página). Pode demorar para volumes grandes.">
                  {exportBusy === 'zip'
            ? <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin"/>
            : <Download className="h-3.5 w-3.5 mr-1"/>}
                  Exportar ZIP (filtro)
                  {exportBusy === 'zip' && exportProgress && (<span className="ml-1 tabular-nums text-[11px] opacity-80">
            ...
            setZipOpen(true);
        }} className="border-slate-200 text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 transition-colors duration-200" title="Baixar ZIP escolhendo um intervalo específico de datas (diálogo dedicado)">
            ...
            <CardContent className="p-0">
              <div ref={recibosScrollRef} className={`overflow-auto ${recibosFiltrados.length > 50 ? 'max-h-[70vh]' : ''}`}>
                <Table>
                  <TableHeader className="sticky top-0 z-10 bg-card">
                    <TableRow>
                      <TableHead className="w-10">
                        <Checkbox aria-label="Selecionar todos os recibos filtrados" checked={recibosFiltrados.length > 0 && selectedRecibos.size === recibosFiltrados.length} onCheckedChange={toggleSelAllRec}/>
                      </TableHead>
                  ...
                  <TableBody>
                    {recibosFiltrados.length === 0 && (<TableRow><TableCell colSpan={9} className="text-center py-8 text-muted-foreground">
                        Sem recibos para os filtros selecionados.
                      </TableCell></TableRow>)}
                    <VirtualRows scrollRef={recibosScrollRef} items={recibosFiltrados} colSpan={9} estimateSize={64} getKey={(r) => r.id} renderRow={(r) => {
            ...
            return (<TableRow key={r.id} data-state={isSel ? 'selected' : undefined} className={r.status === 'cancelado' ? 'opacity-60' : undefined}>
                            <TableCell>
                              <Checkbox aria-label={`Selecionar recibo ${r.numero}`} checked={isSel} onCheckedChange={() => toggleSelRec(r.id)}/>
                            </TableCell>
                            ...
                            <TableCell className="text-xs">
                              <div className="flex flex-col gap-0.5">
                                ...
                                {atrasoDias > 0 && (<Badge variant="outline" className="text-rose-700 border-rose-200 bg-rose-50 w-fit">
                            ...
                            <TableCell className="text-right font-semibold">
                              <div className="flex flex-col items-end gap-0.5">
                                ...
                                {r.status === 'parcial' && (<span className="text-[10px] font-normal text-amber-700">
                                    pago {BRL(Number(r.valorPago || 0))}
                                  </span>)}
                              {r.status === 'cancelado' && (<Button size="sm" variant="outline" onClick={() => voltarParaPendentes(r)} aria-label="Voltar recibo para pendentes" title="Voltar para pendentes — como se não tivesse sido faturado" className="border-primary/30 text-primary hover:bg-primary/10 hover:text-primary dark:border-primary/40 transition-colors duration-200">
                            ...
                            <TableCell className="text-right whitespace-nowrap">
                              <Button size="sm" variant="ghost" onClick={() => setViewContractId(r.contractId)} aria-label="Ver contrato" title="Ver contrato" className="mr-1">
                                <Eye className="h-3.5 w-3.5"/>
                              </Button>
                              <Button size="sm" variant="outline" onClick={() => baixar(r)} aria-label="Baixar PDF">
                                <Download className="h-3.5 w-3.5 mr-1"/> PDF
                              </Button>
                              {(r.status === 'pago' || r.status === 'parcial') && (<Button size="sm" variant="outline" onClick={() => setReabrirDialog(r)} aria-label="Reabrir recibo (marcar como pendente)" title="Reabrir — marcar como pendente" className="border-amber-300 text-amber-700 hover:bg-amber-50 hover:text-amber-800 dark:border-amber-800/60 dark:text-amber-400 dark:hover:bg-amber-950/40 transition-colors duration-200">
                    ...
                    : <MoreVertical className="h-3.5 w-3.5"/>}
                                ...
                                <DropdownMenuContent align="end" className="w-52">
                                  {r.status !== 'cancelado' && r.status !== 'pago' && (<DropdownMenuItem onClick={() => setPayDialog(r)}>
                                      <CheckCircle2 className="h-3.5 w-3.5 mr-2 text-emerald-600"/>
                                      Registrar pagamento
                                    </DropdownMenuItem>)}
                                  {(r.status === 'pago' || r.status === 'parcial') && (<DropdownMenuItem onClick={() => setReabrirDialog(r)}>
                                  ...
                                  {r.status !== 'cancelado' && (<DropdownMenuItem onClick={() => setCancelDialog(r)} className="text-rose-600 focus:text-rose-700">
                                      <XCircle className="h-3.5 w-3.5 mr-2"/>
                                      Cancelar recibo
                                    </DropdownMenuItem>)}
                                  {r.status === 'cancelado' && (<DropdownMenuItem onClick={() => voltarParaPendentes(r)} className="text-primary focus:text-primary">
              ...
              <div className="px-4 pb-3">
                <PaginationBar page={page} pageSize={pageSize} total={totalRecibos} onPageChange={setPage} onPageSizeChange={setPageSize} pageSizeOptions={[25, 50, 100, 200]}/>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
              ...
              <div className="flex items-start justify-between gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                  <div className="relative">
                    ...
                    <Input placeholder="Buscar…" value={search} onChange={(e) => setSearch(e.target.value)} className="h-8 pl-8 w-[200px] text-sm"/>
                  </div>
                </div>
              </div>
            </CardContent>
            <CardContent className="p-0">
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {recibosSemValidade.length === 0 && (<TableRow>
                        <TableCell colSpan={8} className="text-center py-10 text-muted-foreground text-sm">
                          Nenhum recibo sem validade jurídica ainda.
                        </TableCell>
                      </TableRow>)}
                    {recibosSemValidade.map((r) => (<TableRow key={r.id} className={r.status === 'cancelado' ? 'opacity-60' : undefined}>
                ...
                : formatComp(r.competencia)}
                        ...
                        <TableCell className="text-right whitespace-nowrap">
                          ...
                          {(r.status === 'pago' || r.status === 'parcial') && (<Button size="sm" variant="outline" onClick={() => setReabrirDialog(r)} aria-label="Reabrir recibo (marcar como pendente)" title="Reabrir — marcar como pendente" className="ml-1 border-amber-300 text-amber-700 hover:bg-amber-50 hover:text-amber-800 dark:border-amber-800/60 dark:text-amber-400 dark:hover:bg-amber-950/40 transition-colors duration-200">
                              <RefreshCw className="h-3.5 w-3.5 mr-1"/> Reabrir
                            </Button>)}
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
          ...
          <Card>
            ...
            return (<TableRow key={c.name}>
                    ...
                    : <span className="text-muted-foreground">—</span>}
                          ...
                          <TableCell className="text-right">
                            <Badge variant="outline" className={inad >= 30 ? 'border-rose-300 text-rose-700 bg-rose-50 dark:bg-rose-950/40 dark:text-rose-400 dark:border-rose-800/60'
        ...
        <TabsContent value="notas">
          <Card>
            ...
            return (<div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    {kpis.map((k) => {
                    const Icon = k.icon;
                    return (<div key={k.label} className="rounded-lg border border-border/60 bg-gradient-to-br from-indigo-50/60 to-white dark:from-indigo-950/30 dark:to-slate-950 p-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 gap-2">
                <div className="md:col-span-2 relative">
                  <Search className="h-3.5 w-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground"/>
                  <Input value={nfSearch} onChange={(e) => setNfSearch(e.target.value)} placeholder="Buscar por nº NF, cliente, contrato..." className="h-9 pl-8"/>
                </div>
                <select value={nfStatus} onChange={(e) => setNfStatus(e.target.value)} className="h-9 rounded-md border border-input bg-background px-2 text-sm">
                  <option value="all">Todos os status</option>
                  <option value="ativa">Ativas</option>
                  <option value="cancelada">Canceladas</option>
                </select>
                <select value={nfForma} onChange={(e) => setNfForma(e.target.value)} className="h-9 rounded-md border border-input bg-background px-2 text-sm">
                  <option value="all">Toda forma pgto.</option>
                  {Object.entries(INVOICE_FORMA_LABEL).map(([k, v]) => (<option key={k} value={k}>{v}</option>))}
                </select>
                <select value={nfCompanyId} onChange={(e) => setNfCompanyId(e.target.value)} className="h-9 rounded-md border border-input bg-background px-2 text-sm">
            ...
            : <Download className="h-3.5 w-3.5 mr-1"/>}
              ...
              <div className="overflow-auto rounded-md border border-border/60">
                <Table>
                  ...
                  <TableBody>
                    {invoicesLoading ? (<TableRow><TableCell colSpan={9} className="text-center py-8 text-muted-foreground">Carregando notas fiscais…</TableCell></TableRow>) : invoices.length === 0 ? (<TableRow><TableCell colSpan={9} className="text-center py-8 text-muted-foreground">Nenhuma nota fiscal encontrada.</TableCell></TableRow>) : invoices.map(i => (<TableRow key={i.id} className={i.status === 'cancelada' ? 'opacity-60' : ''}>
                ...
                : <Badge className="bg-rose-100 text-rose-700 border-rose-200 border">Cancelada</Badge>}
                        ...
                        <TableCell className="text-right whitespace-nowrap space-x-1">
                          <Button size="sm" variant="ghost" onClick={() => window.open(toAuthedUrl(i.pdfUrl), '_blank', 'noopener,noreferrer')} title="Ver PDF">
                            <Eye className="h-3.5 w-3.5"/>
                          </Button>
                          <Button size="sm" variant="ghost" asChild title="Baixar PDF">
                            <a href={toAuthedUrl(i.pdfUrl)} rel="noopener noreferrer" download={i.pdfOriginalFilename || `nf-${i.numero}.pdf`}>
                              <Download className="h-3.5 w-3.5"/>
                            </a>
                          </Button>

                          {i.status === 'ativa' && (<Button size="sm" variant="ghost" onClick={() => { setNfCancelTarget(i); setNfCancelMotivo(''); }} title="Cancelar NF" className="text-rose-600 hover:bg-rose-50 hover:text-rose-700">
                              <XCircle className="h-3.5 w-3.5"/>
                            </Button>)}
                        </TableCell>
                      </TableRow>))}
              ...
              <PaginationBar page={nfPage} pageSize={nfPageSize} total={nfTotal} onPageChange={setNfPage} onPageSizeChange={setNfPageSize} pageSizeOptions={[25, 50, 100, 200]}/>
            </CardContent>
          </Card>
        </TabsContent>
            ...
            return (<div className="space-y-4">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  <Card><CardContent className="p-3">
                    ...
                    {delta !== null && (<div className={`text-[11px] mt-0.5 flex items-center gap-1 ${delta >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                  ...
                  <CardContent className="p-4 space-y-3">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <div className="relative">
                        ...
                        <Input placeholder="Buscar nº, cliente, empresa…" className="h-9 pl-7 w-64" value={medicoesSearch} onChange={(e) => setMedicoesSearch(e.target.value)}/>
                      </div>
                      {filteredHasFilter && (<Button size="sm" variant="ghost" onClick={() => setMedicoesSearch('')}>
                    ...
                    <div className="rounded-md border overflow-x-auto">
                      <Table>
                        ...
                        <TableBody>
                          {filtered.length === 0 && !medicoesLoading && (<TableRow>
                        ...
                        : <>Nenhuma medição em {formatComp(competencia)}. Clique em <b>Nova medição</b> para começar.</>}
                          ...
                          {filtered.map((m) => (<TableRow key={m.id} className="cursor-pointer hover:bg-muted/40" onClick={() => { setMedicaoViewId(m.id); setMedicaoViewOpen(true); }}>
                        ...
                        try {
                            ...
                            toast.error(e.message);
                                  ...
                                  <Button size="icon" variant="ghost" title="Duplicar" onClick={async () => {
                        try {
                            const full = await medicoesService.get(m.id);
                            const dup = {
                                ...full,
                            ...
                            toast.error(e.message);
                                  ...
                                  <Button size="icon" variant="ghost" title="Editar" onClick={async () => {
                        try {
                            const full = await medicoesService.get(m.id);
                    ...
                    <PaginationBar page={medPage} pageSize={medPageSize} total={medTotal} onPageChange={setMedPage} onPageSizeChange={setMedPageSize} pageSizeOptions={[25, 50, 100, 200]}/>
                  </CardContent>
                </Card>
              </div>);
        })()}
      ...
      {/* Dialogs de Medição */}
      <MedicaoDialog open={medicaoDialogOpen} onOpenChange={setMedicaoDialogOpen} competencia={competencia} periodoInicioDefault={undefined} periodoFimDefault={undefined} editing={medicaoEditing} onSaved={() => loadMedicoes()}/>
      <MedicaoViewDialog medicaoId={medicaoViewId} open={medicaoViewOpen} onOpenChange={setMedicaoViewOpen} onEdit={(m) => {
      ...
      <VincularNfDialog open={!!nfDialogTarget} onOpenChange={(o) => { if (!o)
        setNfDialogTarget(null); }} pending={nfDialogTarget} competencia={competencia} onSuccess={async () => {
      ...
      {/* Cancelar NF */}
      <Dialog open={!!nfCancelTarget} onOpenChange={(o) => { if (!o) {
        ...
        <DialogContent className="max-w-md w-[calc(100vw-2rem)]">
          ...
          <div className="space-y-2">
            ...
            <Textarea id="nf-cancel-motivo" autoFocus value={nfCancelMotivo} onChange={(e) => setNfCancelMotivo(e.target.value)} placeholder="Ex.: NF emitida com dados errados, cliente pediu reemissão…" className="min-h-[90px]" aria-invalid={!nfCancelMotivo.trim()} aria-describedby="nf-cancel-motivo-help"/>
        ...
        }} disabled={!nfCancelMotivo.trim()} className="bg-rose-600 hover:bg-rose-700 text-white">
      ...
      <UnifiedPreviewDialog input={unifPreview} onClose={() => setUnifPreview(null)} onConfirm={confirmUnified} working={batchWorking}/>
      ...
      {selectedRecibos.size > 0 && (<div className="fixed inset-x-0 bottom-0 z-40 pointer-events-none flex justify-center px-3 pb-4 md:pb-6 animate-in slide-in-from-bottom-4 duration-200">
          <div className="pointer-events-auto w-full max-w-3xl rounded-2xl border border-border bg-card/95 backdrop-blur shadow-xl px-3 py-2.5 md:px-4 md:py-3 flex flex-wrap items-center gap-2">
            ...
            <Button size="sm" variant="outline" disabled={batchWorking} onClick={() => exportRecibosCsv(recibosFiltrados.filter(r => selectedRecibos.has(r.id)))} className="transition-colors duration-200">
              <FileSpreadsheet className="h-3.5 w-3.5 mr-1"/> CSV
            </Button>
            <Button size="sm" variant="outline" disabled={batchWorking} onClick={batchReopen} className="border-amber-300 text-amber-700 hover:bg-amber-50 hover:text-amber-800 dark:border-amber-800/60 dark:text-amber-400 dark:hover:bg-amber-950/40 transition-colors duration-200">
              <RefreshCw className="h-3.5 w-3.5 mr-1"/> Reabrir
            </Button>
            <Button size="sm" disabled={batchWorking} onClick={batchMarkPaid} className="bg-emerald-600 hover:bg-emerald-700 transition-colors duration-200">
              {batchWorking
                ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1"/>
                : <CheckCircle2 className="h-3.5 w-3.5 mr-1"/>}
              Marcar pago
            </Button>
            <Button size="sm" variant="outline" disabled={batchWorking} onClick={() => setBatchCancelOpen(true)} className="border-rose-300 text-rose-700 hover:bg-rose-50 hover:text-rose-800 dark:border-rose-800/60 dark:text-rose-400 dark:hover:bg-rose-950/40 transition-colors duration-200">
              <XCircle className="h-3.5 w-3.5 mr-1"/> Cancelar
            </Button>
            <Button size="sm" variant="ghost" onClick={() => setSelectedRecibos(new Set())} className="transition-colors duration-200" aria-label="Limpar seleção">
              <X className="h-4 w-4"/>
            </Button>
          </div>
        </div>)}
      ...
      <Dialog open={batchCancelOpen} onOpenChange={(o) => { if (!o) {
        setBatchCancelOpen(false);
        setBatchCancelMotivo('');
    } }}>
        <DialogContent>
          ...
          <div className="space-y-2">
            ...
            <Textarea id="batch-cancel-motivo" value={batchCancelMotivo} onChange={(e) => setBatchCancelMotivo(e.target.value)} placeholder="Ex.: erro de digitação, duplicidade, renegociação…" className="min-h-[90px]"/>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setBatchCancelOpen(false); setBatchCancelMotivo(''); }}>
              Voltar
            </Button>
            <Button onClick={batchCancelSubmit} disabled={batchWorking || !batchCancelMotivo.trim()} className="bg-rose-600 hover:bg-rose-700 transition-colors duration-200">
      ...
      <PayDialog receipt={payDialog} onClose={() => setPayDialog(null)} onSaved={async () => { setPayDialog(null); await load(); }}/>
      <CancelDialog receipt={cancelDialog} onClose={() => setCancelDialog(null)} onCanceled={async () => { setCancelDialog(null); await load(); }}/>
      ...
      <EditVencimentoDialog receipt={editVencDialog} onClose={() => setEditVencDialog(null)} onSaved={async () => { setEditVencDialog(null); await load(); }}/>

      {/* Exportar recibos por período em ZIP */}
      <Dialog open={zipOpen} onOpenChange={(o) => { if (!zipBusy)
        setZipOpen(o); }}>
        <DialogContent>
          <DialogHeader>
          ...
          <div className="grid gap-4 py-2">
            ...
            <label className="flex items-center gap-2 text-sm text-foreground select-none">
              <Checkbox checked={zipIncludeSV} onCheckedChange={(v) => setZipIncludeSV(Boolean(v))} disabled={zipBusy}/>
              Incluir recibos sem validade jurídica
            </label>
            {zipProgress && (<div className="text-xs text-muted-foreground">
      ...
      <ContractViewDialog contractId={viewContractId} onClose={() => setViewContractId(null)}/>
...
const StatusBadge = ({ r }) => {
    if (r.status === 'cancelado')
        return <Badge variant="outline" className="text-slate-500 border-slate-300">Cancelado</Badge>;
    if (r.status === 'pago')
        return null;
    if (r.status === 'parcial')
...
// ========================= KPI / chip =========================
const KPI = ({ label, value, sub, icon: Icon, accent }) => (<Card className="border-0 shadow-md overflow-hidden">
    <CardContent className="p-0">
...
const QuickChip = ({ active, tone, onClick, children }) => {
    ...
    else if (active && tone === 'amber')
        cls = 'bg-amber-500 text-white border-amber-500 hover:bg-amber-600';
    else if (active)
        ...
        <div className="h-[260px] w-full">
          <ResponsiveContainer>
            <ComposedChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              ...
              <Tooltip formatter={(v) => BRL(Number(v))} contentStyle={{ borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 12 }}/>
...
const GerarReciboPopover = ({ pending, working, competencia, onConfirm, children }) => {
    ...
    const dataInicioContrato = pending.dataInicio ? pending.dataInicio.slice(0, 10) : '';
    return (<Popover open={open} onOpenChange={(o) => !working && setOpen(o)}>
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="rounded-md border border-border/60 bg-muted/30 px-3 py-2 space-y-2">
            <label htmlFor={`gr-per-${pending.contractId}`} className="flex items-start gap-2 cursor-pointer">
              <Checkbox id={`gr-per-${pending.contractId}`} checked={overridePeriodo} onCheckedChange={(v) => setOverridePeriodo(!!v)} className="mt-0.5"/>
            ...
            {overridePeriodo ? (<div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <Label className="text-[10px] text-muted-foreground uppercase">Início</Label>
                  <Input type="date" value={perIniManual} onChange={(e) => setPerIniManual(e.target.value)} className="h-8 text-xs"/>
                </div>
                <div className="space-y-1">
                  <Label className="text-[10px] text-muted-foreground uppercase">Fim</Label>
                  <Input type="date" value={perFimManual} onChange={(e) => setPerFimManual(e.target.value)} className="h-8 text-xs"/>
                </div>
              </div>) : (<div className="rounded-md px-2 py-1.5 border bg-muted/20 border-border/40 space-y-1">
                <div className="text-sm font-semibold text-foreground tabular-nums">
                  {formatPeriodo(periodoPadrao.inicio, periodoPadrao.fim)}
                </div>
                {dataInicioContrato && (<div className="text-[10px] text-muted-foreground flex items-center gap-1">
          ...
          <div className="rounded-md border border-border/60 bg-muted/30 px-3 py-2 space-y-2">
            <label htmlFor={`gr-ov-${pending.contractId}`} className="flex items-start gap-2 cursor-pointer">
              <Checkbox id={`gr-ov-${pending.contractId}`} checked={overrideVenc} onCheckedChange={(v) => setOverrideVenc(!!v)} className="mt-0.5"/>
              <div className="text-[11px] leading-snug">
            ...
            {overrideVenc && (<Input type="date" value={vencManual} onChange={(e) => setVencManual(e.target.value)} className="h-8 text-sm"/>)}
          </div>

          {/* Toggle: sem validade jurídica */}
          <label htmlFor={`gr-sv-${pending.contractId}`} className={'flex items-start gap-2 rounded-md border px-3 py-2 cursor-pointer transition-colors duration-200 ' +
            (semValidade
                ? 'border-amber-300 bg-amber-50 dark:border-amber-800/50 dark:bg-amber-950/30'
                : 'border-border/60 bg-muted/30 hover:bg-muted/50')}>
            <Checkbox id={`gr-sv-${pending.contractId}`} checked={semValidade} onCheckedChange={(v) => setSemValidade(!!v)} className="mt-0.5"/>
            <div className="text-[11px] leading-snug">
          ...
          <div className="flex items-center justify-end gap-2 pt-0.5">
            <Button size="sm" variant="ghost" onClick={() => setOpen(false)} disabled={working} className="h-8 transition-colors duration-200">
              Cancelar
            </Button>
            ...
            (overridePeriodo && (!/^\d{4}-\d{2}-\d{2}$/.test(perIniManual) || !/^\d{4}-\d{2}-\d{2}$/.test(perFimManual)))} className={'h-8 transition-colors duration-200 ' +
            (semValidade
...
const ReceiptUnifiedActions = ({ handleUnifiedAction }) => {
    return (<div className="grid grid-cols-2 gap-2 mt-4">
      <Button size="sm" variant="outline" className="w-full text-amber-600 border-amber-200 hover:bg-amber-50" onClick={() => handleUnifiedAction(true)}>
        Unificado (Sem Validade)
      </Button>
      <Button size="sm" variant="outline" className="w-full text-emerald-600 border-emerald-200 hover:bg-emerald-50" onClick={() => handleUnifiedAction(false)}>
        Unificado (Comum)
      </Button>
    </div>);
};
const UnifiedPreviewDialog = ({ input, onClose, onConfirm, working }) => {
    ...
    if (!input)
        return null;
    return (<Dialog open={!!input} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-4 py-2">
          ...
          <div>
            ...
            <Textarea value={obs} onChange={e => setObs(e.target.value)} placeholder="Ex: Referente a medição extra de agosto..." rows={2}/>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-lg border text-sm">
            ...
            <div className="max-h-32 overflow-auto space-y-1 pr-2">
              {input.items.map((it, idx) => (<div key={idx} className="flex justify-between items-center text-[11px] border-b border-slate-200 dark:border-slate-800 pb-1">
        ...
        <DialogFooter>
          ...
          <Button disabled={working} className={input.semValidade ? 'bg-amber-600 hover:bg-amber-700' : 'bg-emerald-600 hover:bg-emerald-700'} onClick={() => onConfirm({
            ...input,
            dataEmissao,
            dataVencimento,
...
const PayDialog = ({ receipt, onClose, onSaved }) => {
    ...
    useEffect(() => {
        if (!receipt)
            return;
        setForma(receipt.formaPagamento || 'pix');
    ...
    return (<Dialog open={!!receipt} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <SearchableSelect value={forma} onValueChange={(v) => setForma(v)} placeholder="Forma" options={Object.keys(FORMA_LABEL).map(k => ({
            value: k,
            label: FORMA_LABEL[k],
        }))}/>
            </div>
          ...
          {parcial && (<div>
              ...
              <Input type="number" step="0.01" min={0} max={total} value={valor} onChange={e => setValor(Number(e.target.value))}/>
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={saving || valorFinal <= 0} className="bg-emerald-600 hover:bg-emerald-700">
...
// ========================= CancelDialog =========================
const CancelDialog = ({ receipt, onClose, onCanceled }) => {
    ...
    return (<Dialog open={!!receipt} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-md">
        ...
        <div>
          ...
          <Textarea value={motivo} onChange={e => setMotivo(e.target.value)} rows={3} placeholder="Ex.: erro de emissão, duplicado, cliente cancelou contrato…"/>
        </div>
        <DialogFooter>
...
const EditVencimentoDialog = ({ receipt, onClose, onSaved }) => {
    const [dataEmissao, setDataEmissao] = useState('');
    const [dataVencimento, setDataVencimento] = useState('');
    ...
    const salvar = async () => {
        ...
        if (!isDate(dataEmissao)) {
            toast.error('Data de emissão inválida');
            return;
        }
        if (dataVencimento && !isDate(dataVencimento)) {
            toast.error('Data de vencimento inválida');
            return;
        }
        if (periodoInicio && !isDate(periodoInicio)) {
            toast.error('Início do período inválido');
            return;
        }
        if (periodoFim && !isDate(periodoFim)) {
            toast.error('Fim do período inválido');
            return;
        }
        if (periodoInicio && periodoFim && periodoFim < periodoInicio) {
            toast.error('Fim do período deve ser >= início');
            return;
        }
    ...
    return (<Dialog open={!!receipt} onOpenChange={(o) => !o && !busy && onClose()}>
      <DialogContent className="max-w-lg">
        ...
        <div className="grid gap-3 py-2 sm:grid-cols-2">
          ...
          <div className="space-y-1">
            ...
            <Input id="edit-valor" type="number" min={0} step="0.01" value={valor} onChange={(e) => setValor(e.target.value)}/>
          </div>
          <div className="space-y-1">
            <Label htmlFor="edit-numdisp">Número exibido (opcional)</Label>
            <Input id="edit-numdisp" value={numeroDisplay} onChange={(e) => setNumeroDisplay(e.target.value)} placeholder={receipt?.numero || ''} maxLength={64}/>
          </div>
        </div>
...
function GastosPanel() {
    ...
    const gastosScrollRef = useRef(null);
    ...
    const loadCats = useCallback(async () => {
        try {
            setCategories(await expenseCategoriesService.list());
        }
        catch { /* silencioso */ }
    }, []);
    const loadRec = useCallback(async () => {
        try {
            setRecurring(await recurringExpensesService.list());
        }
        catch { /* silencioso */ }
    }, []);
    useEffect(() => { load(); }, [load]);
    ...
    const fetchAllFiltered = useCallback(async (hardLimit = 5000) => {
        const PAGE = 200;
    ...
    const submit = async () => {
        if (!form.descricao || form.valor == null) {
    ...
    const remove = async (e) => {
        ...
        if (!(await confirmDialog({ description: 'Excluir este gasto?', destructive: true })))
            return;
        try {
    ...
    return (<div className="space-y-4">
      ...
      <Card>
        <CardContent className="p-4 flex flex-wrap gap-3 items-end">
          ...
          <div>
            ...
            <SearchableSelect value={cat} onValueChange={setCat} triggerClassName="h-9 w-[180px]" placeholder="Categoria" searchPlaceholder="Buscar categoria..." options={[
            { value: 'all', label: 'Todas' },
          ...
          <div>
            ...
            <SearchableSelect value={origem} onValueChange={(v) => setOrigem(v)} triggerClassName="h-9 w-[160px]" placeholder="Origem" options={[
            { value: 'all', label: 'Todas' },
          ...
          <div className="flex-1 min-w-[200px]">
            <Label className="text-xs">Buscar</Label>
            <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Descrição, fornecedor, NF…" className="h-9"/>
          </div>
          <div className="ml-auto flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setRecOpen(true)}>
              ...
              {recurring.filter(r => r.ativo).length > 0 && (<Badge variant="secondary" className="ml-2">{recurring.filter(r => r.ativo).length}</Badge>)}
            </Button>
      ...
      {recurring.filter(r => r.ativo).length > 0 && (<Card className="border-indigo-100 bg-indigo-50/40">
      ...
      <Card>
        <CardContent className="p-0">
          <div ref={gastosScrollRef} className={`overflow-auto ${list.length > 50 ? 'max-h-[70vh]' : ''}`}>
            <Table>
              <TableHeader className="sticky top-0 z-10 bg-card">
                <TableRow>
              ...
              <TableBody>
                {loading && (<TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">
                    <Loader2 className="h-4 w-4 inline animate-spin mr-2"/> Carregando…
                  </TableCell></TableRow>)}
                {!loading && list.length === 0 && (<TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">
                    Nenhum gasto no período.
                  </TableCell></TableRow>)}
                <VirtualRows scrollRef={gastosScrollRef} items={list} colSpan={8} estimateSize={48} getKey={(e) => `${e.origem || 'm'}-${e.id}`} renderRow={(e) => (<TableRow key={`${e.origem || 'm'}-${e.id}`}>
      ...
      <PaginationBar page={page} pageSize={pageSize} total={total} onPageChange={setPage} onPageSizeChange={setPageSize} pageSizeOptions={[25, 50, 100, 200]}/>
      ...
      <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) {
        ...
        <DialogContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <Label className="text-xs">Categoria</Label>
              <SearchableSelect value={form.categoria} onValueChange={(v) => setForm({ ...form, categoria: v })} placeholder="Categoria" searchPlaceholder="Buscar categoria..." options={activeCats.map(c => ({ value: c.key, label: c.label }))}/>
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex items-center gap-2">
                <Input type="file" accept="image/*,application/pdf,.doc,.docx,.xls,.xlsx" onChange={async (e) => {
            ...
            try {
                ...
                {form.anexoUrl && (<>
                    <a href={toAbsoluteUrl(form.anexoUrl)} target="_blank" rel="noreferrer" className="text-xs text-indigo-600 underline whitespace-nowrap">ver anexo</a>
                    <Button type="button" variant="ghost" size="sm" onClick={() => setForm({ ...form, anexoUrl: '' })} aria-label="Remover anexo">
                      <Trash2 className="h-3.5 w-3.5"/>
                    </Button>
                  </>)}
              </div>
      ...
      <CategoriesDialog open={catsOpen} onClose={() => setCatsOpen(false)} categories={categories} onChanged={loadCats}/>
      <RecurringDialog open={recOpen} onClose={() => setRecOpen(false)} list={recurring} categories={activeCats} onChanged={async () => { await loadRec(); await load(); }}/>
    </div>);
}
// ========================= CategoriesDialog =========================
const CategoriesDialog = ({ open, onClose, categories, onChanged }) => {
    ...
    const add = async () => {
        ...
        setSaving(true);
        try {
            await expenseCategoriesService.create({ label: label.trim() });
            setLabel('');
            await onChanged();
        }
        catch (e) {
    ...
    const toggle = async (c) => {
        try {
            await expenseCategoriesService.update(c.id, { ativo: !c.ativo });
            await onChanged();
        }
        catch (e) {
            toast.error(e.message);
    ...
    const remove = async (c) => {
        if (!(await confirmDialog({ description: `Excluir categoria "${c.label}"?`, destructive: true })))
            return;
        try {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex items-end gap-2">
          <div className="flex-1">
            ...
            <Input value={label} onChange={e => setLabel(e.target.value)} placeholder="Ex.: Marketing" onKeyDown={(e) => e.key === 'Enter' && add()}/>
          </div>
        ...
        <div className="border rounded-md divide-y max-h-[280px] overflow-y-auto">
          {categories.length === 0 && (<div className="text-xs text-slate-400 text-center py-6">Nenhuma categoria.</div>)}
          {categories.map(c => (<div key={c.id} className="flex items-center justify-between px-3 py-2 text-sm">
              <div className="flex items-center gap-2">
...
const RecurringDialog = ({ open, onClose, list, categories, onChanged }) => {
    ...
    const save = async () => {
        ...
        try {
            if (editing.id)
                await recurringExpensesService.update(editing.id, editing);
            else
                await recurringExpensesService.create(editing);
            toast.success('Salvo');
            setEditing(null);
            await onChanged();
        }
    ...
    const remove = async (r) => {
        if (!(await confirmDialog({ description: `Excluir recorrência "${r.descricao}"?`, destructive: true })))
            return;
        try {
            await recurringExpensesService.remove(r.id);
    ...
    const toggle = async (r) => {
        try {
            await recurringExpensesService.update(r.id, { ativo: !r.ativo });
            await onChanged();
        }
        catch (e) {
            toast.error(e.message);
      ...
      <DialogContent className="sm:max-w-2xl">
        ...
        {!editing && (<>
            ...
            <div className="border rounded-md divide-y max-h-[340px] overflow-y-auto">
              {list.length === 0 && (<div className="text-xs text-slate-400 text-center py-6">Nenhuma recorrência cadastrada.</div>)}
              {list.map(r => (<div key={r.id} className="flex items-center justify-between px-3 py-2 gap-2">
        ...
        {editing && (<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input type="number" min={1} max={31} value={editing.diaMes ?? 1} onChange={e => setEditing({ ...editing, diaMes: Math.min(31, Math.max(1, Number(e.target.value) || 1)) })}/>
            </div>
            <div>
              <Label className="text-xs">Categoria</Label>
              <SearchableSelect value={editing.categoria || 'outros'} onValueChange={(v) => setEditing({ ...editing, categoria: v })} placeholder="Categoria" searchPlaceholder="Buscar categoria..." options={categories.map(c => ({ value: c.key, label: c.label }))}/>
            </div>
            <div>
              <Label className="text-xs">Fornecedor</Label>
