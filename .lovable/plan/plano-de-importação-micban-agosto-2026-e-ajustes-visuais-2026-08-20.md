...
- Substituir o texto longo de "SYSTEM RULES" pelo novo texto solicitado: "🚀 Iniciando importação de contratos (Micban - Agosto 26)...\n\n❌ Erro na importação: 85 | \n\n86 |   const client = await pool.connect();\n\n87 |   try {\n\n88 |     const micbanRes = await client.query(\"SELECT id FROM erp_companies WHERE razao_social ILIKE '%Micban%' LIMIT 1\");\n\n89 |     if (micbanRes.rows.length === 0) {\n\n90 |       throw new Error('Empresa emissora \"Micban\" não encontrada no sistema.');\n\n                     ^\n\nerror: Empresa emissora \"Micban\" não encontrada no sistema.\n\n      at importFromExcel (/var/www/rota-azul-viagens/scripts/import-agosto-micban.ts:90:17)\n\nAntes de executar qualquer ação, faça de 2 a 5 perguntas INTERATIVAS para entender melhor o que eu quero (alvo, valores, escopo) — não digite o texto das perguntas na sua resposta; quero elas interativas, para eu marcar ou responder direto na caixa de perguntas. Não implemente nada até eu responder. Se o pedido já estiver totalmente especificado (alvo + valor + escopo), aí sim execute direto."
